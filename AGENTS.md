...
- `backend/` — Flask API (SQLite via Flask-SQLAlchemy). Organized as: `backend/models/` (SQLAlchemy models: `Category`, `Transaction`), `backend/schemas/` (payload validation + response serialization), `backend/services/` (internal business logic/db operations, not exposed), and `backend/api/v1/` (thin endpoint handlers, one file per resource). Routes: `/api/categories` (CRUD + search/sort/filter/stats), `/api/transactions` (CRUD + type/category/date filters), `/api/dashboard` (KPIs per week/month/year, category breakdown, category-vs-monthly-average, month-vs-previous-years, 12-month history), `/api/health`. Run/tests use the repo-root `venv/` (Python 3.13.3).
...
- `venv\Scripts\python -m pytest backend\tests` — backend tests (63 tests covering category, transaction, dashboard aggregation, and export rules).
...
- Backend DB file is `backend/finapp.db` (git-ignored). Models use a composite FK on `(category_id, type)` so a transaction's type must match its category's type at the DB level; category delete/type-change rules are enforced in `backend/services/category_service.py`, transaction validation in `backend/services/transaction_service.py`. SQLite FK enforcement is enabled via a PRAGMA listener in `backend/extensions.py` (SQLite defaults to FKs off). Endpoint files in `backend/api/v1/` stay thin — business logic lives in `backend/services/`, parsing/serialization in `backend/schemas/`.
